...
image = imread('Match-Mismatchforplanar.tif');
...
image = imread('Match-MismatchforEEG.tif');
imagearray(198+2*size(imcropped,1):197+size(image,1)+2*size(imcropped,1),420+2*size(imcropped,2)+size(image,2):419+2*size(imcropped,2)+2*size(image,2),:) = image;
