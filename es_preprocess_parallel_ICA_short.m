function es_preprocess_parallel_ICA_short(step,prevStep,p,pathstem,subjects,subjcnt,dates,blocksin,blocksout,rawpathstem,badeeg)
...
switch prevStep
    ...
    case 'convert'
        prevStep = 'spm8*.mat';
    case 'ICA_artifacts'
        prevStep = 'Mdsspm8*.mat';
    case 'downsample'
        prevStep = 'dsspm8*.mat';
    case 'epoch'
        prevStep = 'e*Mdsspm8*.mat';
    case 'merge'
        prevStep = 'c*Mdsspm8*.mat';
    case 'rereference'
        prevStep = 'Md*Mdsspm8*.mat';
    case 'TF_power'
        prevStep = 'tf*Mdsspm8*.mat';
    case 'TF_phase'
        prevStep = 'tph*Mdsspm8*.mat';
    case 'TF_rescale'
        prevStep = 'r*Mdsspm8*.mat';
    case 'filter'
        prevStep = 'f*Mdsspm8*.mat';
    case 'baseline'
        prevStep = 'b*Mdsspm8*.mat';
    case 'average'
        prevStep = 'm*Mdsspm8*.mat';
    case 'weight'
        prevStep = 'w*Mdsspm8*.mat';
    case 'combineplanar'
        prevStep = 'p*Mdsspm8*.mat';
    case 'artefact'
        prevStep = 'a*Mdsspm8*.mat';
    case 'blink'
        prevStep = 'clean*Mdsspm8*.mat';
    case 'image'
        prevStep = 'trial*Mdsspm8*.img';
    case 'smooth'
        prevStep = 'sm*Mdsspm8*.img';
    case 'firstlevel'
        prevStep = 't*Mdsspm8*.img';
...
switch step
    ...
    case 'downsample' % filter data
        ...
        for s=1:size(subjects,1)
            ...
            % search for input files
            filesdone = dir(['dsspm8*.mat']);    %Check if already done!
            files = dir(prevStep);
    ...
    case 'ICA_artifacts'
        ...
        % search for input files
        filesdone = dir(['Mdsspm8*.mat']);    %Check if already done!
        files = dir(prevStep);
