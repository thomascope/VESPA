...
for m=1:length(modality)
    ...
    for s=1:length(subjects)
        ...
        try
            D = spm_eeg_load([bwd subjects{s} '/' file2plot]);
        catch
            D = spm_eeg_load([bwd subjects{s} '/' file2plotelse]);
        end
        times = D.time*1000;
        ...
        chanind = D.indchantype(modality{m});
...
% parameters
file2plot = [bwd subjects{1} '/' file2plot];
...
for c=1:length(con2plot)
    ...
    if sum(strcmp(chanlabel,{'MEG' 'MEGPLANAR' 'EEG'}))
        ...
        % get channels indices for specified modality
        chaninds = D.indchantype(chanlabel);
        % get condition index
    ...
    % units)
    if sum(strcmp(chanlabel,'MEG')) || ~isempty(intersect(D.indchantype('MEG'),D.indchannel(chanlabel))) % MEGMAG
        measure = 'Amplitude (fT)';
        data = data * 1e15;
    elseif sum(strcmp(chanlabel,'MEGPLANAR')) || ~isempty(intersect(D.indchantype('MEGPLANAR'),D.indchannel(chanlabel))) % MEGPLANAR
        measure = 'Amplitude (fT/m)';
        data = data * 1e15;
    elseif sum(strcmp(chanlabel,'EEG')) || ~isempty(intersect(D.indchantype('EEG'),D.indchannel(chanlabel))) % EEG
...
for s=1:length(subjects)
    ...
    file = dir([bwd subjects{s} '/wcfm*.mat']);
    %file = dir([bwd subjects{s} '/cfm*.mat']);
    filename = [bwd subjects{s} '/' file.name];
...
if ~isempty(intersect(D.indchantype('MEG'),D.indchannel(chanlabels{1}))) % MEGMAG
    measure = 'Amplitude (fT)';
    data = data * 1e15;
elseif ~isempty(intersect(D.indchantype('MEGPLANAR'),D.indchannel(chanlabels{1}))) % MEGPLANAR
    measure = 'Amplitude (fT/m)';
    data = data * 1e15;
elseif ~isempty(intersect(D.indchantype('EEG'),D.indchannel(chanlabels{1}))) % EEG
    measure = 'Amplitude (\muV)';
    data = data * 1e6;
end
...
D = spm_eeg_load([bwd 'gwpcfmcfbMdspm8_01_preTest_raw_ssst.mat']); % load grand average
...
for c=1:nconditions
    ...
    for w=1:nwindows
        ...
        cfg.xlim = windows(w,:);
        cfg.trials = D.indtrial(con2plot{c});
        ...
        if ~isempty(imgdir)
            ...
            [Cind Cind_ind] = intersect(Cind,D.indchantype(modality));
            Cel = Cel(Cind_ind,:);
