...
epoch  = [-100 400];    % EPOCH around subject responses for averaging (milliseconds)
...
%         end
        ...
        spm('defaults', 'eeg');
        if exist('edfMmmn_resp.mat','file')
        else
            S = [];
            S.pretrig = epoch(1);
            S.posttrig = epoch(2);
    ...
    if exist('aedfMmmn_resp.mat','file')
        D = spm_eeg_load('aedfMmmn_resp.mat');
    else
        S = []; S.D = D.fname;
        ...
        S.methods(end).settings.threshold = 2500e-12; %2500ft%meg_thr(ss);
        %
        S.methods(end+1).fun = 'threshchan';
        S.methods(end).channels = 'MEGPLANAR';
        S.methods(end).settings.threshold = 900e-12; %900ft%meg_thr(ss);
        %
        S.methods(end+1).fun = 'threshchan';
    ...
    if exist('maedfMmmn_resp.mat','file')
        D = spm_eeg_load('maedfMmmn_resp.mat');
    else
