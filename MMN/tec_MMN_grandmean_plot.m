...
for ty = 1:length(chanlabel)
    ...
    for c=1:length(con2plot)
        ...
        %        chaninds = D.meegchannels(chanlabel{ty});
...
%         coninds = D.pickconditions(con2plot{c});
    ...
    % units)
    if sum(strcmp(chanlabel{ty},'MEG')) || ~isempty(intersect(D.meegchannels('MEG'),D.indchannel(chanlabel{ty}))) % MEGMAG
        measure = 'Amplitude (fT)';
        datacont = datacont * 1e15;
    elseif sum(strcmp(chanlabel{ty},'MEGPLANAR')) || ~isempty(intersect(D.meegchannels('MEGPLANAR'),D.indchannel(chanlabel{ty}))) % MEGPLANAR
        measure = 'Amplitude (fT/m)';
        datacont = datacont * 1e15;
    elseif sum(strcmp(chanlabel{ty},'EEG')) || ~isempty(intersect(D.meegchannels('EEG'),D.indchannel(chanlabel{ty}))) % EEG
        measure = 'Amplitude (\muV)';
        datacont = datacont * 1e6;
    end
    
    % get channels indices for specified modality
    chaninds = S.meegchannels(chanlabel{ty});
    % get condition index
    coninds = S.pickconditions(con2plot{c});
    % get data for selected channels and current condition
...
for ty = 1:length(chanlabel)
    figure
        ...
        chaninds = D.meegchannels(chanlabel{ty});
        % get condition index
        firstinds = D.pickconditions(con2contrast{1});
        secondinds = D.pickconditions(con2contrast{2});
        % get data for selected channels and current condition
