function es_preprocess_parallel(step,prevStep,p,pathstem,subjects,subjcnt,dates,blocksin,blocksout,rawpathstem,badeeg)
...
switch step
    ...
    case 'downsample' % filter data
        ...
        for s=1:size(subjects,1)
            ...
            % search for input files
            filesdone = dir(['deMspm8*.mat']);    %Check if already done!
            files = dir(prevStep);
...
%         %Nperm = 1000; %The Nperm above took more than a week!!
%         Nperm = 0; % the new ICA script uses a template - no longer required!
...
%         %ICAFiltPars = [40]; %if try to bandpass, often fails even with dimension reduction
        ...
        modalities = {'MEGMAG' 'MEGPLANAR'}; %no template file for EEG yet
       ...
       % search for input files
        filesdone = dir(['Mspm8*.mat']);    %Check if already done!
        files = dir(prevStep);
        ...
        if isempty(filesdone)
            ...
            for f=1:length(files)
                ...
                for a = 1:length(ref_chans)
                    ...
                    save('/group/language/data/thomascope/vespa/lasterror.mat','subjects','files','f')
                    end
                end
                % figure,plot(refs')
        ...
        elseif length(filesdone) < length(files)   %Assumes that file list is in the same order between runs!
            ...
            for f=length(filesdone):length(files)
                ...
                for m = 1:length(modalities)
                    ...
                    ICA.FiltPars = [ICAFiltPars D.fsample];
    ...
    case 'smooth'
        ...
        for m=1:length(modality)
            ...
            for s=1:size(subjects,1) % for multiple subjects
                ...
                filePath = [pathstem subjects '/' modality{m}];
                cd(filePath);
        ...
        for w=1:size(windows,1)
            ...
            for s=1:size(subjects,1) % for multiple subjects
                ...
                for m=1:length(modality)
                    ...
                    folders = dir([pathstem subjects '/' modality{m} '/type*']);
