function jp_spm8_surfacerender2_version_es(img, cmap, cfg, img2, cmap2)
...
if ~isempty(img2) % es edit- for overlaying 2nd image
    ...
    % if [min max] colorscale specified, trim data to only display > min value
    if ~isempty(cfg.colorscale2) && ~strcmp(cfg.colorscale2, 'symmetrical')
        Ysiz2 = size(Y2);
        YY2 = reshape(Y2,1,numel(Y2));
        YY2(YY2 < cfg.colorscale2(1)) = NaN;
...
if any(v(:))|any(v2(:))
  ...
  elseif ~isempty(cfg.colorscale)
      ...
      cdat = squeeze(ind2rgb(floor( (v(:)-cmin)/(cmax) * size(col,1)), col));
      fprintf('Color scale from %g to %g.\n', cmin, cmax);
      cmin2 = cfg.colorscale2(1);
      cmax2 = cfg.colorscale2(2);
