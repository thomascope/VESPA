...
if isfield(S,'covariates')
    ...
    for thisone = 1:length(S.covariates)
        SPM.xC(end+1).rc = S.covariates{thisone}.value;
        SPM.xC(end).rcname = S.covariates{thisone}.name;
        SPM.xC(end).iCC = 5; %No centring - perhaps implement later
        SPM.xC(end).iCFI = 1; %No interaction - I don't know how to implement this yet
        SPM.xC(end).type = 1; %Covariate of interest
        SPM.xC(end).cols = SPM.nscan+thisone;
        SPM.xC(end).c = S.covariates{thisone}.value;
        SPM.xC(end).cname = {S.covariates{thisone}.name};
    end
else
    SPM.xC  = [];	
end
