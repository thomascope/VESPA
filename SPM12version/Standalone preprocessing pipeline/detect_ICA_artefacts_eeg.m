function [sphere,weights,ICs] = detect_ICA_artefacts_eeg(S);
...
try
    ...
    try
        ...
        %[weights,sphere,compvars,bias,signs,lrates,ICs] = runica(d,'pca',PCA_dim,'extended',1,'maxsteps',10); %for DEBUG only
...
if length(FiltPars) == 3
    ...
    fprintf('Bandpass filtering fails too often, so sequentially filtering highpass at from %d Hz then lowpass at to %d Hz \n',FiltPars(1), FiltPars(2));
    for r=1:Nrefs
        refs.tem{r} = ft_preproc_highpassfilter(refs.tem{r}, FiltPars(3), FiltPars(1),  [], 'but','twopass','reduce');
        refs.tem{r} = ft_preproc_lowpassfilter(refs.tem{r}, FiltPars(3), FiltPars(2),  [], 'but','twopass','reduce');
    end
    ICs  = ft_preproc_highpassfilter(ICs,  FiltPars(3), FiltPars(1),  [], 'but','twopass','reduce'); %Highpass filter first to avoid ringing artifacts
    ICs  = ft_preproc_lowpassfilter(ICs,  FiltPars(3), FiltPars(2),  [], 'but','twopass','reduce')'; %Don't transpose twice!
