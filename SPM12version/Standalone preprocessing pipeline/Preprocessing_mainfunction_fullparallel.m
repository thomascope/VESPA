function Preprocessing_mainfunction(step,prevStep,p,pathstem,maxfilteredpathstem, subjects,subjcnt,dates,blocksin,blocksout,rawpathstem,badeeg,badmeg,testvar)
...
switch step
    ...
    case 'downsample' % filter data
        ...
        for s=1:size(subjects,1)
            ...
            % search for input files
            filesdone = dir(['d*Mrun*.mat']);    %Check if already done!
            files = dir(prevStep);
        ...
        if isempty(filesdone)
            ...
            for f=testvar
                ...
                for m = 1:length(modalities)
                    if exist(['MEG_ICs_' num2str(m) '_' files(f).name])
                        load ['MEG_ICs_' num2str(m) '_' files(f).name]
                        m = m+1
                    elseif exist(['MEG_ICs_' m '_' files(f).name])
                        load (['MEG_ICs_' m '_' files(f).name])
                        m = m+1
                    end
                    %Do this twice for both MEG sensor types
                    if exist(['MEG_ICs_' num2str(m) '_' files(f).name])
                        load ['MEG_ICs_' num2str(m) '_' files(f).name]
                        m = m+1
                    elseif exist(['MEG_ICs_' m '_' files(f).name])
                    ...
                    if exist([['ICs_for_' files(f).name]])
                        error(['File' num2str(testvar) 'previously ICA_artifacted for ' subjects '!\n\n'])
                    end
