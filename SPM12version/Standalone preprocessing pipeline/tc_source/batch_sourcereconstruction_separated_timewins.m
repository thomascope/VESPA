...
%TEC's groups:
% 1 = MSPgroup with prior locations based on Sohoglu, 2 = MSPgroup without prior for symmetry, 3 = MSPgroup (planar only), 4 = LORgroup 35-950ms (only grads), 5 = LORgroup 35-950ms (all modalities), 6 = LORgroup
...
% This version redoeos the MSP reconstruction within windows, rather than
% doing one inversion for the whole window and the restricting the
% frequency bands. Should help pull out the frontal source

all_windows = [90 130; 180 240; 270 420; 450 700; 750 900];
for vprime = 2
    for windows = 1:5
        val = 20+windows;
        woi = all_windows(windows,:);
        %val = vprime;
        ...
        %% Compute grand-average of source images
            ...
            imagetype = {
                ['fmcfbMdeMrun1_raw_ssst_' num2str(val) '_t' num2str(woi(1)) '_' num2str(woi(2)) '_f1_40*'];
                };
            imagetype_split = {
                ['fmcfbMdeMrun1_1_raw_ssst_' num2str(val) '_t' num2str(woi(1)) '_' num2str(woi(2)) '_f1_40*'];
                };
