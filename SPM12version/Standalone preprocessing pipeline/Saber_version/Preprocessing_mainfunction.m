function Preprocessing_mainfunction(step,prevStep,p,pathstem,maxfilteredpathstem, subjects,subjcnt,dates,blocksin,blocksout,rawpathstem,badeeg)
...
switch prevStep
    ...
    case 'ICA_artifacts'
        prevStep = 'MTest*.mat';
    case 'downsample'
        prevStep = 'd*MTest*.mat';
    case 'epoch'
        prevStep = 'eMTest*.mat';
    case 'merge'
        prevStep = 'c*eMTest*.mat';
    case 'rereference'
        prevStep = 'Md*eMTest*.mat';
    case 'TF_power'
        prevStep = 'tf*eMTest*.mat';
    case 'TF_phase'
        prevStep = 'tph*eMTest*.mat';
    case 'TF_rescale'
        prevStep = 'r*eMTest*.mat';
    case 'filter'
        prevStep = 'f*eMTest*.mat';
    case 'baseline'
        prevStep = 'b*eMTest*.mat';
    case 'average'
        prevStep = 'm*eMTest*.mat';
    case 'weight'
        prevStep = 'w*eMTest*.mat';
    case 'combineplanar'
        prevStep = 'p*eMTest*.mat';
    case 'artefact'
        prevStep = 'a*eMTest*.mat';
    case 'blink'
        prevStep = 'clean*eMTest*.mat';
    case 'image'
        prevStep = 'trial*eMTest*.img';
    case 'smooth'
        prevStep = 'sm*eMTest*.img';
    case 'firstlevel'
        prevStep = 't*eMTest*.img';
...
switch step
    ...
    case 'downsample' % filter data
        ...
        for s=1:size(subjects,1)
            ...
            % search for input files
            filesdone = dir(['deMTest*.mat']);    %Check if already done!
            files = dir(prevStep);
...
%                     S = [];
    ...
    case 'ICA_artifacts'
        %addpath('/imaging/ss06/sandbox/Tom_files/eeglab13_3_2b');
        %addpath('/imaging/local/eeglab/');
        
        addpath(genpath('/home/tc02/matlab/eeglab13_3_2b'))
        rmpath('/home/tc02/matlab/eeglab13_3_2bfunctions/octavefunc/signal')
        %Put here the path of a version of EEGLAB that has the fileIO toolbox (currently imaging/local version is too old)
        %addpath(genpath('/imaging/local/software/fieldtrip/fieldtrip-current'))
        %eeglab;
        % spm eeg

        % %
        ICA.PCA_dim = 70;                    % Number of PCs for ICA - for ADJUST routine, should be set to at least nEEG channels (70 in my dataset)
        ...
        ICA.Nperm = 0;                              % turn off bootstrapping
        ...
        ref_chans = {'EOG061','EOG062'}; %,'MISC001','MISC002','MISC003'};
       ...
       % search for input files
        %filesdone = [];    %For debug
        filesdone = dir(['MTest*.mat']);    %Check if already done! In case it crashes after one or two runs, to save you having to repeat everything.
        files = dir(prevStep);
        ...
        if isempty(filesdone)
            ...
            %for f = 1
                ...
                ICA.subj = subjects;
                if strcmp(ICA.subj, 'meg14_0234_vc10'); %Because there are so many missing electrodes for this subject, ADJUST will fail and crash the parallel pool
                ...
                for m = 1:length(modalities)
                    ...
                    if strcmp(ICA.mod, 'EEG');
                        ...
                        ICA.refs.spa = {arttopos.HEOG{m}', arttopos.VEOG{m}'};  % ECG too variable?
                    ...
                    if strcmp(ICA.mod, 'EEG');
                        ...
                        % rejection, run interTest_ADJ and manually create
                        % a cell matrix called remove{} containting a
                        % rejected component per cell
                        remove{m} = interTest_ADJ_noplot(EEG,'report');
        ...
        elseif length(filesdone) < length(files)   %Assumes that file list is in the same order between runs!
            fprintf(['\n\n' 'first ' num2str(length(filesdone)) ' files previously ICA_artifacted for ' subjects '!\n\n']);
            for f=length(filesdone):length(files)
                D = spm_eeg_load(files(f).name);
                refs = []; % Reference signal for correlating with ICs
                for a = 1:length(ref_chans)
                ...
                if strcmp(ICA.subj, 'meg14_0234_vc10'); %Because there are so many missing electrodes for this subject, ADJUST will fail
                    modalities = {'MEGMAG' 'MEGPLANAR'};
                end
                ...
                for m = 1:length(modalities)
                    ...
                    if strcmp(ICA.mod, 'EEG');
                        ...
                        ICA.refs.spa = {arttopos.HEOG{m}', arttopos.VEOG{m}', []};  % ECG too variable?
                        if length(ICA.refs.tem) == 2
                            ICA.refs.spa = ICA.refs.spa{1:2}
                        elseif length(ICA.refs.spa) ~= length(ICA.refs.tem)
                            error('The number of reference channels (EOG, ECG etc.) is less than expected. Please edit this section of code appropriately')
                        end
                    end
                ...
                % Remove artifacts from the data using the translation
                % matrix TraMat
            ...
            fprintf(['\n\nData previously ICA_artifacted for ' subjects '!\n\n']);
        end
...
%             load([pathstem p.montage_rerefname]); % load montage
        ...
        for s=1:size(subjects,1)
            ...
            for f=1:length(files)
                ...
                good = setxor(D.indchantype('EEG'),bad); % rereference (excluding bad channels)
