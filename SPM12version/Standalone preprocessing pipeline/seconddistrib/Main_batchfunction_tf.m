...
parfor cnt = 1:size(subjects,2)
    Preprocessing_mainfunction_fullparallel_eegrankreduced('downsample','ICA_artifacts',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
parfor cnt = 1:size(subjects,2)
...
p.freq = [48 52];
parfor cnt = 1:size(subjects,2)
    Preprocessing_mainfunction_fullparallel_eegrankreduced('filter','filter',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
parfor cnt = 1:size(subjects,2)
   Preprocessing_mainfunction_fullparallel_eegrankreduced('epoch','secondfilter',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt,dates,blocksin,blocksout,rawpathstem, badeeg);
end
parfor cnt = 1:size(subjects,2)
    Preprocessing_mainfunction_fullparallel_eegrankreduced('merge','epoch',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
parfor cnt = 1:size(subjects,2)
...
parfor cnt = 1:size(subjects,2) 
    Preprocessing_mainfunction_fullparallel_eegrankreduced('combineplanar','fmceffbMdMr*.mat',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end

Preprocessing_mainfunction_fullparallel_eegrankreduced('grand_average','pfmceffbMdMr*.mat',p,pathstem, maxfilteredpathstem, subjects);
...
parfor cnt = 1:size(subjects,2)    
   Preprocessing_mainfunction_fullparallel_eegrankreduced('weight','pfmceffbMdMr*.mat',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end

Preprocessing_mainfunction_fullparallel_eegrankreduced('grand_average','wpfmceffbMdMr*.mat',p,pathstem, maxfilteredpathstem, subjects);
% This saves the grand weighted average file for each group in the folder of the
% first member of that group. For convenience, you might want to move them
% to separate folders.
parfor cnt = 1:size(subjects,2)
    Preprocessing_mainfunction_fullparallel_eegrankreduced('image','fmceffbMdMr*.mat',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
parfor cnt = 1:size(subjects,2)
    % The input for smoothing should be the same as the input used to make
    % the image files.
    Preprocessing_mainfunction_fullparallel_eegrankreduced('smooth','fmceffbMdMr*.mat',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
for cnt = 1
    % The input for smoothing should be the same as the input used to make
    % the image files. Only need to do this for a single subject
    Preprocessing_mainfunction_fullparallel_eegrankreduced('mask','fmceffbMdMr*.mat',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end  
...
parfor cnt = 1:size(subjects,2)
    Preprocessing_mainfunction_fullparallel_eegrankreduced('TF','merge',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
parfor cnt = 1:size(subjects,2)
    Preprocessing_mainfunction_fullparallel_eegrankreduced('average','TF_power',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
p.robust = 0; %robust averaging doesn't work for phase data
parfor cnt = 1:size(subjects,2)
    Preprocessing_mainfunction_fullparallel_eegrankreduced('average','TF_phase',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
p.robust = 1; % just in case we want to do any more averaging later
%TF_rescale to baseline correct the induced power data only
parfor cnt = 1:size(subjects,2)
    Preprocessing_mainfunction_fullparallel_eegrankreduced('TF_rescale','mtf*dMrun*.mat',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
Preprocessing_mainfunction_fullparallel_eegrankreduced('grand_average','TF_rescale',p,pathstem, maxfilteredpathstem, subjects);
parfor cnt = 1:size(subjects,2)    
   Preprocessing_mainfunction_fullparallel_eegrankreduced('weight','TF_rescale',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
Preprocessing_mainfunction_fullparallel_eegrankreduced('grand_average','wrmtf*.mat',p,pathstem, maxfilteredpathstem, subjects);
parfor cnt = 1:size(subjects,2)
    Preprocessing_mainfunction_fullparallel_eegrankreduced('image','TF_rescale',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
parfor cnt = 1:size(subjects,2)
    % The input for smoothing should be the same as the input used to make
    % the image files.
    Preprocessing_mainfunction_fullparallel_eegrankreduced('smooth','TF_rescale',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
for cnt = 1
    % The input for smoothing should be the same as the input used to make
    % the image files. Only need to do this for a single subject
    Preprocessing_mainfunction_fullparallel_eegrankreduced('mask','TF_rescale',p,pathstem, maxfilteredpathstem, subjects{cnt},cnt);
end
