...
function remove = interface_ADJ_noplot (EEG,report)
...
%         EEGep = pop_rmbase( EEGep, [0  0]);
   ...
   [art, horiz, vert, blink, disc,...
    ...
    remove = unique([blink, disc, horiz, vert]);

    save (nome, 'blink', 'horiz', 'vert', 'disc', 'remove');
...
%%
   ...
   [art, horiz, vert, blink, disc,...
    ...
    remove = unique([eb, hem, vem, gd, aic]);
    save (nome, 'eb', 'hem', 'vem', 'gd' , 'aic', 'remove');
