...
fnames = char('sm_condition_Match_4_run.nii','sm_condition_Match_8_run.nii','sm_condition_Match_16_run.nii','sm_condition_Mismatch_4_run.nii','sm_condition_Mismatch_8_run.nii','sm_condition_Mismatch_16_run.nii');
fnames_nobuttons = char('sm_condition_Match_4_nobuttons.nii','sm_condition_Match_8_nobuttons.nii','sm_condition_Match_16_nobuttons.nii','sm_condition_Mismatch_4_nobuttons.nii','sm_condition_Mismatch_8_nobuttons.nii','sm_condition_Mismatch_16_nobuttons.nii');
...
for s = 1:length(subjects)
    ...
    try
        congruency_contrast = spm_imcalc(fnames_nobuttons,'congruency_contrast_nobuttons.nii','(i4+i5+i6)-(i1+i2+i3)');
    catch
    end
    ...
    elseif group(s) == 3
        ...
        xlim([start_vox size(C_fup{thiscon_fup},2)])
        try
            C_fup_nobuttons{thiscon_fup} = spm_read_vols(spm_vol('congruency_contrast_nobuttons.nii'));
            [rows, ~, ~] = size(C_fup_nobuttons{thiscon_fup});
            figure(conscomb)
            subplot(9,3,3*(thiscon_fup-1)+3);
            [rows, ~, ~] = size(C_fup_nobuttons{thiscon_fup});
            newHeight = [1 3 * rows];
            %imshow(flipud(C_fup{thispat}), 'YData', newHeight);
            imagesc(-flipud(C_fup_nobuttons{thiscon_fup}),[-1.2 1.2])
            colormap('jet')
            %Find max contrast and the index of where within 80% of max contrast
            all_maxs = max((C_fup_nobuttons{thiscon_fup}(1:13,:)));
            abs_max = max(all_maxs);
            all_times = find(all_maxs>(abs_max*0.8));
            con_times_fup_nobuttons(thiscon_fup) = min(all_times)*congruency_contrast.mat(2,2)+congruency_contrast.mat(2,4);
            set(gca,'xtick',[min(all_times)],'xticklabel',num2str(con_times_fup_nobuttons(thiscon_fup)))
            set(gca,'ytick',[1 39],'yticklabel',[80 2])
            set(gcf,'color','w');
            xlim([start_vox size(C_fup_nobuttons{thiscon_fup},2)])
        catch
        end
    ...
    elseif group(s) == 4
        ...
        xlim([start_vox size(P_fup{thispat_fup},2)])
        try
            P_fup_nobuttons{thispat_fup} = spm_read_vols(spm_vol('congruency_contrast_nobuttons.nii'));
            [rows, ~, ~] = size(P_fup_nobuttons{thiscon_fup});
            figure(patscomb)
            subplot(9,3,3*(thispat_fup-1)+3);
            [rows, ~, ~] = size(P_fup_nobuttons{thispat_fup});
            newHeight = [1 3 * rows];
            %imshow(flipud(C_fup{thispat}), 'YData', newHeight);
            imagesc(-flipud(P_fup_nobuttons{thispat_fup}),[-1.2 1.2])
            colormap('jet')
            %Find max contrast and the index of where within 80% of max contrast
            all_maxs = max((P_fup_nobuttons{thispat_fup}(1:13,:)));
            abs_max = max(all_maxs);
            all_times = find(all_maxs>(abs_max*0.8));
            pat_times_fup_nobuttons(thispat_fup) = min(all_times)*congruency_contrast.mat(2,2)+congruency_contrast.mat(2,4);
            set(gca,'xtick',[min(all_times)],'xticklabel',num2str(pat_times_fup_nobuttons(thispat_fup)))
            set(gca,'ytick',[1 39],'yticklabel',[80 2])
            set(gcf,'color','w');
            xlim([start_vox size(P_fup_nobuttons{thispat_fup},2)])
        catch
        end
    end
