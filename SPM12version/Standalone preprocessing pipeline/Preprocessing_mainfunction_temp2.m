function Preprocessing_mainfunction_temp2(step,prevStep,p,pathstem,maxfilteredpathstem, subjects,subjcnt,dates,blocksin,blocksout,rawpathstem,badeeg,badmeg,testvar)
...
switch prevStep
    ...
    case 'downsample'
        prevStep = 'd*Mrun*.mat';
    case 'epoch'
        prevStep = 'e*dMrun*.mat';
    case 'merge'
        prevStep = 'c*dMrun*.mat';
    case 'rereference'
        prevStep = 'M*dMrun*.mat';
    case 'TF_power'
        prevStep = 'tf*dMrun*.mat';
    case 'TF_phase'
        prevStep = 'tph*dMrun*.mat';
    case 'TF_rescale'
        prevStep = 'r*dMrun*.mat';
    case 'filter'
        prevStep = 'fb*dMrun*.mat';
    case 'secondfilter'
        prevStep = 'ffb*dMrun*.mat';
    case 'baseline'
        prevStep = 'b*dMrun*.mat';
    case 'average'
        prevStep = 'm*dMrun*.mat';
    case 'weight'
        prevStep = 'w*dMrun*.mat';
    case 'combineplanar'
        prevStep = 'p*dMrun*.mat';
    case 'artefact'
        prevStep = 'a*dMrun*.mat';
    case 'blink'
        prevStep = 'clean*dMrun*.mat';
    case 'image'
        prevStep = 'trial*dMrun*.img';
    case 'smooth'
        prevStep = 'sm*dMrun*.img';
    case 'firstlevel'
        prevStep = 't*dMrun*.img';
...
switch step
    ...
    case 'downsample' % filter data
        ...
        for s=1:size(subjects,1)
            ...
            % search for input files
            filesdone = dir(['dMrun*.mat']);    %Check if already done!
            files = dir(prevStep);
    ...
    case 'ICA_artifacts'
        ...
        % search for input files
        %filesdone = [];    %For debug
        filesdone = dir(['Mrun*.mat']);    %Check if already done! In case it crashes after one or two runs, to save you having to repeat everything.
        files = dir(prevStep);
            ...
            for f=testvar
                ...
                for m = 1:length(modalities)
                    ...
                    if strcmp(ICA.mod, 'EEG');
                        ...
                        % a cell matrix called remove{} containting a
                        % rejected component per cell
                        % This version also removes the warning and failure that
                        % occurred if the number of PCA dimensions was less
                        % than the number of channels, because sometimes
                        % channels are insufficiently independent for the
                        % extraction if independent components.
                        remove{m} = interface_ADJ_noplot(EEG,'report');
                        %interface_ADJ(EEG,'report')
    ...
    case 'ICA_artifacts_tf'
        ...
        ICA.FiltPars_orig = [0.1 100];              % filter bandpass [1 40] %% [0.05 20];
        ICA.TemAbsPval = .05/ICA.PCA_dim;      %0.05;  % Too liberal if not permuted?
        ICA.SpaAbsPval = .05/ICA.PCA_dim;        %0.05;  % Too liberal if not permuted?
        ICA.TemRelZval = 3;                    % Relative temporal threshold in Z-values
        ICA.SpaRelZval = 2;                     % 3 is too strict? (since high topo correlation anyway)
    ...
    case 'average' % average data
        ...
        if p.robust == 1;
            ...
            S.robust.ks = 3;
            S.robust.removebad = 1; %This messes up later visualisation, but is safer. Sets badchannel data to NaN
        else
