...
function plot_betas_overall_thresholded(threshold,modality,contrastnumber,varargin)
hold on
...
controltoplot = rms(data{1}(data{1}.selectchannels(modality),:,contrastnumber));
...
patienttoplot = rms(data{2}(data{2}.selectchannels(modality),:,contrastnumber));
...
title(['Overall RMS Sensor power for ' modality ', contrast ' num2str(contrastnumber) ],'fontsize',20)
...
H = downsample(H,4);
if nanmean(patienttoplot(logical(H)))>nanmean(controltoplot(logical(H)))
    jbfill(-500:4:1500,patienttoplot,zeros(1,length(patienttoplot)),H,'k','k',[],0.3);
else
    jbfill(-500:4:1500,controltoplot,zeros(1,length(patienttoplot)),H,'k','k',[],0.3);
end
end
save_string = ['./Significant_peaks/Overall RMS Sensor power for ' modality ', contrast ' num2str(contrastnumber) '.pdf'];
...
for i = 1:length(varargin)
    fieldtrip_topoplot_thresholded(varargin{i},contrastnumber,threshold,modality)
    scales(:,i) = caxis;
end
