...
function plot_betas_tf_previsualbaseline_thresholded(plot_threshold,whichbetas,contrast,statspm,nsubj,frequency,modality,varargin)

pathstem = ['/imaging/mlr/users/tc02/vespa/preprocess/SPM12_fullpipeline_fixedICA/stats_tf_previsualbaselinecorrectedsm_/combined_-100_1000_' modality];
...
save_string = ['./Significant_peaks/tf_previsual_' titlestr{2}(2:end) ', contrast ' num2str(contrast) ' for ' modality ' at ' num2str(frequency) ' Hz.pdf'];
eval(['export_fig ''' save_string ''' -transparent'])

for i = 1:length(starting)
    timewindow = [times(starting(i)) times(ending(i))];
    thisscale = fieldtrip_topoplot_highlight_tf_nosave(timewindow,contrastnumber,frequency,modality);
    scales(i) = max(max(abs(thisscale)));
    fieldtrip_topoplot_highlight_tf_thresholded(timewindow,contrastnumber,frequency,modality,[-scales(i), scales(i)])
end
...
for i = 1:length(varargin)
    fieldtrip_topoplot_highlight_tf_previsual_thresholded(varargin{i},contrastnumber,frequency,modality,plot_threshold)
end