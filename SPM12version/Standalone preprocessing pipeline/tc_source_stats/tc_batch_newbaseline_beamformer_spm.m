function tc_batch_newbaseline_beamformer_spm(~)
...
inputstem = '/imaging/mlr/users/tc02/vespa/preprocess/SPM12_fullpipeline_fixedICA/newbaseline_tf_source/final_eLORETA_trunkated_restricted/';
...
for s=1:length(controls2average) % specify file locations for batch_spm_anova_vES
    ...
    for con=1:length(conditions)
        ...
        files{1}{s}{con} = [inputstem controls2average{s} '/uv_pow_cond_' conditions{con} '_ceffbMdMrun1_raw_ssst.nii'];
        if exist(files{1}{s}{con},'file')
        else
            files{1}{s}{con} = [inputstem controls2average{s} '/uv_pow_cond_' conditions{con} '_ceffbMdMrun1_1_raw_ssst.nii'];
        end
...
for s=1:length(patients2average) % specify file locations for batch_spm_anova_vES
    ...
    for con=1:length(conditions)
        ...
        files{2}{s}{con} = [inputstem patients2average{s} '/uv_pow_cond_' conditions{con} '_ceffbMdMrun1_raw_ssst.nii'];
        if exist(files{2}{s}{con},'file')
        else
            files{2}{s}{con} = [inputstem patients2average{s} '/uv_pow_cond_' conditions{con} '_ceffbMdMrun1_1_raw_ssst.nii'];
        end
    end
