function varargout=sigstar(groups,stats,offset,interaction,nosort)
    ...
    y=ylim;
    yd=myRange(y)*0.07; %separate sig bars vertically by 5% 

    thisY = zeros(1,length(groups));
    for ii=1:length(groups)
        thisY(ii)=findMinY(xlocs(ii,:))+yd;
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY(ii),stats(ii),interaction);
    end
    %-----------------------------------------------------
    ...
    for ii=1:length(groups)
        ...
        set(H(ii,1),'YData',y)
        if interaction == 1
            H((ii+length(groups)),:)=makeSignificanceBar([xlocs(ii,1)-offset, xlocs(ii,1)+offset],thisY(ii)-yd,1,2);
            H((ii+(2*length(groups))),:)=makeSignificanceBar([xlocs(ii,2)-offset, xlocs(ii,2)+offset],thisY(ii)-yd,1,2);
            y=get(H((ii+length(groups)),1),'YData');
            y(1)=y(1)-yd;
            y(4)=y(4)-yd;
            set(H((ii+length(groups)),1),'YData',y)
            y=get(H((ii+(2*length(groups))),1),'YData');
            y(1)=y(1)-yd;
            y(4)=y(4)-yd;
            set(H((ii+(2*length(groups))),1),'YData',y)
        end
    end
...
function H=makeSignificanceBar(x,y,p,interaction)
