...
for s = 1    
    ...
    for n = 1:11
        ...
        temp_granger_data = nan(nchans,nchans,ntimes,36,2,nptmp);
        
        granger_data = temp_granger_data;
        %coh = find(All_Conds==8);    %XXX Find out what this means and how to address it! All_Conds doesn't exist so must have been loaded.
        coh = matchconds; %XXX Guess for now, but can't be right as this would mean rctrl == 0
        ...
        for p = 1:nptmp
            ...
            if n == 11
                rcoh = coh;
                rctrl = mismatchconds; % XXX Is this right? I don't necessarily just want the contrast though
            end
                ...
                for chan_from = 1:(nchans - 1)% For loops for every channel pair combination %XXX Does this do both directions??
                    for chan_to = (chan_from+1):nchans
                        ...
                        for cond = 1:2    % For loop for each condition (figure vs control)
                            ...
                            if cond == 1
                                ftdata.trial = permute(double(S.data(chans,tIdx,rcoh)),[3,1,2]); %if cond == 1 then this line. If cond == 2 then look for All_Conds==80 %XXX check that elements in correct order
                            elseif cond == 2
                                ftdata.trial = permute(double(S.data(chans,tIdx,rctrl)),[3,1,2]);
                            end
                            ...
                            fres    = 2.5 * ones(size(foi)); %XXX what is this? Frequency smoothing?
