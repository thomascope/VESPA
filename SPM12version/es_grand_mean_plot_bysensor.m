...
subjects_and_parameters_nospmlaunch
...
all_chanlabels = {'MEG1613'; 'MEG0343'; 'MEG0232'}
% Right STG 1613
% Right MFG 0343
% Right ITG 0232
for chanset = 1:length(all_chanlabels)
    ...
    for p = 1:2
        ...
        for t=1:nticks % plot bar graph
            bar(t,means(1,t),char(legcolours(t)),'LineWidth',2);
        end
        
        for t=1:nticks % add error bars
            if means(1,t) < 0
                errorhandle = errorbar(t,means(1,t),sems(1,t),0,'k','LineWidth',2);
            else
                errorhandle = errorbar(t,means(1,t),0,sems(1,t),'k','LineWidth',2);
            end
        ...
        if mean(means(1,:)) < 0 % if mean across conditions negative, reverse y axis
            set(gca,'YDir','Reverse');
        end
        ylabel(measure);
    ...
    barweb(reshape(patientmeans,2,4)',reshape(patientsems,2,4)',[],{'90-130ms'; '180-240ms'; '270-420ms'; '450-700ms'},sprintf(['Patient response by time at %s, a sensor of type %s'],chanlabels, char(D.chantype(chaninds))),[],'fT',[],[],{'Match','Mismatch'}) ;
    legend('Match','Mismatch','location','NorthWest');
    if mean(controlmeans) < 0
    set(gca,'YDir','Reverse');
    end
    ...
    figure
    set(gcf,'position',[100,100,1200,800])
    barweb(reshape(controlmeans,2,4)',reshape(controlsems,2,4)',[],{'90-130ms'; '180-240ms'; '270-420ms'; '450-700ms'},sprintf(['Control response by time at %s, a sensor of type %s'],chanlabels, char(D.chantype(chaninds))),[],'fT',[],[],{'Match','Mismatch'}) ;
    legend('Match','Mismatch','location','NorthWest');
    if mean(controlmeans) < 0
    set(gca,'YDir','Reverse');
    end
    ...
    if size(windows,1) == 4
        ...
        for timewin = 1:2
            ...
            figure
            set(gcf,'position',[100,100,1200,800])
            barweb(reshape(patientmeans,2,3)',reshape(patientsems,2,3)',[],ticks,sprintf(['Patient response by sensory detail at %s, a sensor of type %s'],chanlabels, char(D.chantype(chaninds))),[],'fT',[],[],{'Match','Mismatch'}) ;
            legend('Match','Mismatch','location','NorthWest');
            if mean(controlmeans) < 0
                set(gca,'YDir','Reverse');
            end
            ...
            figure
            set(gcf,'position',[100,100,1200,800])
            barweb(reshape(controlmeans,2,3)',reshape(controlsems,2,3)',[],ticks,sprintf(['Control response by sensory detail at %s, a sensor of type %s'],chanlabels, char(D.chantype(chaninds))),[],'fT',[],[],{'Match','Mismatch'}) ;
            legend('Match','Mismatch','location','NorthWest');
            if mean(controlmeans) < 0
                set(gca,'YDir','Reverse');
            end
...
for chanset = 1:length(all_chanlabels)
    ...
    figure
    set(gcf,'position',[100,100,1200,800])
    barweb(reshape(patientmeans,2,4)',reshape(patientsems,2,4)',[],{'90-130ms'; '180-240ms'; '270-420ms'; '450-700ms'},sprintf(['Patient response by time at %s, a sensor of type %s'],chanlabels, char(D.chantype(chaninds))),[],'fT',[],[],{'Clear','Unclear'}) ;
    ...
    figure
    set(gcf,'position',[100,100,1200,800])
    barweb(reshape(controlmeans,2,4)',reshape(controlsems,2,4)',[],{'90-130ms'; '180-240ms'; '270-420ms'; '450-700ms'},sprintf(['Control response by time at %s, a sensor of type %s'],chanlabels, char(D.chantype(chaninds))),[],'fT',[],[],{'Clear','Unclear'}) ;
    legend('Clear','Unclear','location','NorthWest');
    if mean(controlmeans) < 0
        set(gca,'YDir','Reverse');
    end
...
%             figure
%             set(gcf,'position',[100,100,1200,800])
%             barweb(reshape(patientmeans,2,3)',reshape(patientsems,2,3)',[],ticks,sprintf(['Patient response at %s, a sensor of type %s'],chanlabels, char(D.chantype(chaninds))),[],'fT',[],[],{'Clear','Unclear'});
%             legend('Clear','Unclear','location','NorthWest');
%             if mean(controlmeans) < 0
%             set(gca,'YDir','Reverse');
%             end
...
%             figure
%             set(gcf,'position',[100,100,1200,800])
%             barweb(reshape(controlmeans,2,3)',reshape(controlsems,2,3)',[],ticks,sprintf(['Control response at %s, a sensor of type %s'],chanlabels, char(D.chantype(chaninds))),[],'fT',[],[],{'Clear','Unclear',}) ;
%             legend('Clear','Unclear','location','NorthWest');
%             if mean(controlmeans) < 0
%             set(gca,'YDir','Reverse');
%             end
