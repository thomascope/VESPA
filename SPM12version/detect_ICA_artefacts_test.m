...
try
    ...
    try
        [weights,sphere,compvars,bias,signs,lrates,ICs] = rik_runica(d,'pca',PCA_dim,'extended',1,'maxsteps',10,'rseed',Rseed); % Just local copy where rand seed can be passed
    catch
        if ~isempty(Rseed); warning('Random seed requested but no facility with standard runica?'); end
        [weights,sphere,compvars,bias,signs,lrates,ICs] = runica(d,'pca',PCA_dim,'extended',1,'maxsteps',10);
    end
end
