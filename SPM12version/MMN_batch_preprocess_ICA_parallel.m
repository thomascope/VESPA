...
%     workerpool.ResourceTemplate='-l nodes=^N^,mem=8GB,walltime=336:00:00';
...
%     MMN_preprocess_parallel_ICA('definetrials','maxfilter',p,pathstem,subjects{cnt},cnt);
%      MMN_preprocess_parallel_ICA('convert+epoch','maxfilter',p,pathstem,subjects{cnt},cnt,dates,blocksin,blocksout,rawpathstem,badeeg);
   MMN_preprocess_parallel_ICA('convert','maxfilter',p,pathstem,subjects{cnt},cnt,dates,blocksin,blocksout,rawpathstem,badeeg);
   MMN_preprocess_parallel_ICA('ICA_artifacts','convert',p,pathstem,subjects{cnt},cnt);
   MMN_preprocess_parallel_ICA('definetrials','maxfilter',p,pathstem,subjects{cnt},cnt);
   MMN_preprocess_parallel_ICA('epoch','ICA_artifacts',p,pathstem,subjects{cnt},cnt,dates,blocksin,blocksout,rawpathstem,badeeg);
    MMN_preprocess_parallel_ICA('downsample','epoch',p,pathstem,subjects{cnt},cnt);
    MMN_preprocess_parallel_ICA('rereference','downsample',p,pathstem,subjects{cnt},cnt);
    MMN_preprocess_parallel_ICA('baseline','rereference',p,pathstem,subjects{cnt},cnt);
    MMN_preprocess_parallel_ICA('filter','baseline',p,pathstem,subjects{cnt},cnt);
    MMN_preprocess_parallel_ICA('merge','filter',p,pathstem,subjects{cnt},cnt);
    MMN_preprocess_parallel_ICA('sort','merge',p,pathstem,subjects{cnt},cnt);
    MMN_preprocess_parallel_ICA('average','merge',p,pathstem,subjects{cnt},cnt);
    MMN_preprocess_parallel_ICA('filter','average',p,pathstem,subjects{cnt},cnt);
    
    MMN_preprocess_parallel_ICA('combineplanar','fm*.mat',p,pathstem,subjects{cnt},cnt);
   % MMN_preprocess_parallel_ICA('grand_average','pcfm*.mat',p,pathstem,subjects{cnt},cnt);
    
    %MMN_preprocess_parallel_ICA('weight','pcfm*.mat',p,pathstem,subjects{cnt},cnt);
    %MMN_preprocess_parallel_ICA('grand_average','wpcfm*.mat',p,pathstem,subjects{cnt},cnt);
    
    %MMN_preprocess_parallel_ICA('image','cfm*.mat',p,pathstem,subjects{cnt},cnt);
    %MMN_preprocess_parallel_ICA('image','wpcfm*.mat',p,pathstem,subjects{cnt},cnt);
    %MMN_preprocess_parallel_ICA('smooth','image',p,pathstem,subjects{cnt},cnt);
    %MMN_preprocess_parallel_ICA('firstlevel','smooth',p,pathstem,subjects{cnt},cnt);
    
    %MMN_preprocess_parallel_ICA('mask','image',p,pathstem,subjects{cnt},cnt);
    
    %% Other preprocessing steps
    %MMN_preprocess_parallel_ICA('artefact','baseline',p,pathstem,subjects{cnt},cnt);
    %MMN_preprocess_parallel_ICA('sort','average',p,pathstem,subjects{cnt},cnt);
    %MMN_preprocess_parallel_ICA('erase','',p,pathstem,subjects{cnt},cnt);
    %p.outputstem = '/imaging/es03/P6E1/preprocess_TF/'; % for copying files
    %MMN_preprocess_parallel_ICA('copy','rereference',p,pathstem,subjects{cnt},cnt);
