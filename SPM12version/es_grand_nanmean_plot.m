...
% parameters
patientfile2plot = '/imaging/mlr/users/tc02/vespa/preprocess/SPM12_MEG_artifactsonly/Grandnanmeans/patients_grandnanmean.mat';
controlfile2plot = '/imaging/mlr/users/tc02/vespa/preprocess/SPM12_MEG_artifactsonly/Grandnanmeans/controls_grandnanmean.mat';
...
nanmeans = reshape(nanmeans,nticks,npanels).';
...
for p=1:npanels
    ...
    for t=1:nticks % plot bar graph
        bar(t,nanmeans(p,t),char(legcolours(t)),'LineWidth',2);
    end
    
    for t=1:nticks % add error bars
        if nanmeans(p,t) < 0
            errorhandle = errorbar(t,nanmeans(p,t),sems(p,t),0,'k','LineWidth',2);
        else
            errorhandle = errorbar(t,nanmeans(p,t),0,sems(p,t),'k','LineWidth',2);
        end
    ...
    if nanmean(nanmeans(p,:)) < 0 % if nanmean across conditions negative, reverse y axis
        set(gca,'YDir','Reverse');
    end
    ylabel(measure);
