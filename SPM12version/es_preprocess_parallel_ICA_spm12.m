function es_preprocess_parallel(step,prevStep,p,pathstem,maxfilteredpathstem, subjects,subjcnt,dates,blocksin,blocksout,rawpathstem,badeeg)
...
switch prevStep
    ...
    case 'merge'
        prevStep = 'c*erun*.mat';
    case 'rereference'
        prevStep = 'Md*erun*.mat';
    case 'TF_power'
        prevStep = 'tf*erun*.mat';
    case 'TF_phase'
        prevStep = 'tph*erun*.mat';
    case 'TF_rescale'
        prevStep = 'r*erun*.mat';
    case 'filter'
        prevStep = 'f*erun*.mat';
    ...
    case 'artefact'
        prevStep = 'a*erun*.mat';
    case 'blink'
        prevStep = 'clean*erun*.mat';
    case 'image'
        prevStep = 'trial*erun*.img';
    case 'smooth'
        prevStep = 'sm*erun*.img';
    case 'firstlevel'
        prevStep = 't*erun*.img';
...
switch step
    ...
    case 'downsample' % filter data
        ...
        for s=1:size(subjects,1)
            ...
            % search for input files
            filesdone = dir(['derun*.mat']);    %Check if already done!
            files = dir(prevStep);
            ...
            for s=1:size(subjects,1) % for multiple subjects
                ...
                if exist('channel_labels','var')
                    if strcmp(channel_labels,'all')
                        S.images.elecs = D.indchantype(S.modality);
                    else
