function MMN_preprocess_parallel_ICA(step,prevStep,p,pathstem,subjects,subjcnt,dates,blocksin,blocksout,rawpathstem,badeeg)

switch prevStep
    
    case 'maxfilter'
        prevStep = 'mmn*ssst.fif';
    case 'convert'
        prevStep = 'spm8*.mat';
    case 'ICA_artifacts'
        prevStep = 'Mspm8*.mat';
    case 'downsample'
        prevStep = 'd*Mspm8*.mat';
    case 'epoch'
        prevStep = 'e*Mspm8*.mat';
    case 'merge'
        prevStep = 'c*Mspm8*.mat';
    case 'rereference'
        prevStep = 'Md*Mspm8*.mat';
    case 'TF_power'
        prevStep = 'tf*Mspm8*.mat';
    case 'TF_phase'
        prevStep = 'tph*Mspm8*.mat';
    case 'TF_rescale'
        prevStep = 'r*Mspm8*.mat';
    case 'filter'
        prevStep = 'f*Mspm8*.mat';
    case 'baseline'
        prevStep = 'b*Mspm8*.mat';
    case 'average'
        prevStep = 'm*Mspm8*.mat';
    case 'weight'
        prevStep = 'w*Mspm8*.mat';
    case 'combineplanar'
        prevStep = 'p*Mspm8*.mat';
    case 'artefact'
        prevStep = 'a*Mspm8*.mat';
    case 'blink'
        prevStep = 'clean*Mspm8*.mat';
    case 'image'
        prevStep = 'trial*Mspm8*.img';
    case 'smooth'
        prevStep = 'sm*Mspm8*.img';
    case 'firstlevel'
        prevStep = 't*Mspm8*.img';
...
switch step
    ...
    case 'erase'
        ...
        for s=1:size(subjects,1)
            ...
            filePath = [pathstem subjects '/MMN+Rest'];
            cd(filePath);
    ...
    case 'copy'
        ...
        for s=1:size(subjects,1)
            ...
            for f=1:length(files)
                ...
                outputfullpath = [outputstem subjects '/MMN+Rest'];
    ...
    case 'ICA_artifacts'
        ...
        %ICAFiltPars = []; %for no filtering of independent components!
        ICAFiltPars = [0.5 40]; %if try to bandpass, often fails even with dimension reduction
    ...
    case 'image' % assumes merged files (i.e. one per subject)
        ...
        for m=1:length(modality) % for multiple modalities
            ...
            for s=1:size(subjects,1) % for multiple subjects
                ...
                % set input file
                S.D = [pathstem subjects '/MMN+Rest/' files.name];
                ...
                if strcmp(D.transformtype,'time') % time-domain data
                    ...
                    if strcmp(S.images.fmt,'frequency') % create images by averging over frequency
                        ...
                        for fl=1:length(folders)
                            filePath = [pathstem subjects '/MMN+Rest/' modality{m} '/' folders(fl).name];
                            cd(filePath);
                    ...
                    elseif strcmp(S.images.fmt,'channels') % create images by averaging over channels
                        ...
                        for fl=1:length(folders)
                            ...
                            filePath = [pathstem subjects '/MMN+Rest/' modality{m}  '/' newfoldername];
                            cd(filePath);
                            if strfind(files.name,'tph')
    ...
    case 'smooth'
        ...
        for m=1:length(modality)
            ...
            for s=1:size(subjects,1) % for multiple subjects
                ...
                filePath = [pathstem subjects '/MMN+Rest/' modality{m}];
                cd(filePath);
                ...
                if strcmp(modality{m},'Source')
                    ...
                    for fd=1:length(folders)
                        ...
                        filePath = [pathstem subjects '/MMN+Rest/' modality{m} '/' folders(fd).name];
                        cd(filePath);
            ...
            for f=1:length(files)
                ...
                for m=1:length(modality)
                    ...
                    for c=1:length(conditions)
                        ...
                        % main process
                        foldername = [pathstem subjects '/MMN+Rest/' modality{m} '/1D_type_' conditions{c} '/'];
    ...
    case 'mask' % make mask for image files so that stats are done only on time window of interest
        ...
        for m=1:length(modality)
            ...
            % change to input directory (image folder level)
            filePath = [pathstem subjects{1} '/MMN+Rest/' modality{m}];
            cd(filePath);
            ...
            % change to input directory (image file level)
            filePath = [pathstem subjects{1} '/MMN+Rest/' modality{m} '/' folders(1).name];
            cd(filePath);
            ...
            S.image = files.name;
            %S.outfile = [pathstem sprintf(['MMN+Rest/' modality{m} '_mask_%d_%dms.img'],S.window(1),S.window(2))];
            S.outfile = [pathstem sprintf(['MMN+Rest/' modality{m} '_1D_mask_%d_%dms.img'],S.window(1),S.window(2))];
        ...
        for w=1:size(windows,1)
            ...
            for s=1:size(subjects,1) % for multiple subjects
                ...
                for m=1:length(modality)
                    ...
                    folders = dir([pathstem subjects '/MMN+Rest/' modality{m} '/type*']);
